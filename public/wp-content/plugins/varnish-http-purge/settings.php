...
class VarnishStatus {
	...
	public function register_settings() {
		...
		// Auth settings.
        register_setting( 'vhp-settings-auth', 'vhp_varnish_auth', array( &$this, 'settings_auth_sanitize' ) );
        add_settings_section( 'vhp-settings-auth-section', __( 'Configure Auth Key', 'varnish-http-purge' ), array( &$this, 'options_settings_auth' ), 'varnish-auth-settings' );
        add_settings_field( 'varnish_auth_key', __( 'Set Varnish Auth Key', 'varnish-http-purge' ), array( &$this, 'settings_auth_key_callback' ), 'varnish-auth-settings', 'vhp-settings-auth-section' );
    }
    ...
    /**
     * Options Settings - Auth Key
     *
     * @since 4.0
     */
    public function options_settings_auth() {
        ?>
        <p><a name="#configureip"></a><?php esc_html_e( 'There are cases when an Auth Key is needed. For example when you do not know before hand the IP address of the webserver. ', 'varnish-http-purge' ); ?></p>
        <p><?php esc_html_e( 'If varnish is running inside docker you may not have access to the IP address of webserver at the time the VCL is created', 'varnish-http-purge' ); ?></p>
        <?php
    }
     ...
     * @since 4.0
     */
    public function settings_auth_key_callback() {
        $auth_key = get_site_option( 'vhp_varnish_auth' );

        echo '<input type="text" id="vhp_varnish_auth" name="vhp_varnish_auth" value="' . esc_attr( $auth_key ) . '" size="25" '  . '/>';
        echo '<label for="vhp_varnish_ip">';

        esc_html_e( 'Example: ', 'varnish-http-purge' );
        echo '<code>A9JAD74G3L9BAC</code>';
     ...
     * @since 4.0
     */
    public function settings_auth_sanitize( $input ) {

        $output      = '';
        $set_message = __( 'You have entered an invalid Auth Key.', 'varnish-http-purge' );
        $set_type    = 'error';

        if ( empty( $input ) ) {
            return; // do nothing.
        } else {
            $set_message = 'Proxy Auth Key Updated.';
            $set_type    = 'updated';
            $output      = $input;
        }
	...
	public function settings_page() {
		...
		<div class="wrap">
			...
			if ( ! is_multisite() ) {
                ...
                <form action="options.php" method="POST" >
                    <?php
                    settings_fields( 'vhp-settings-auth' );
                    do_settings_sections( 'varnish-auth-settings' );
                    submit_button( __( 'Save Auth Key', 'varnish-http-purge' ), 'secondary' );
                    ?>
                </form>
				<?php
			} else {
